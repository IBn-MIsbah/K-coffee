...
export async function POST(_: Request, { params }: { params: Promise<{ productId: string }> }) {
  try {
    ...
    return Response.json({ product: await setAdminProductActive(actor, (await params).productId, false) });
  } catch (error) {
    ...
    return Response.json({ error: "Unable to archive this product." }, { status: 500 });
  }
}
