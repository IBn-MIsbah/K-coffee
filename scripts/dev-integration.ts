...
if (!sourceUrl)
  throw new Error(
    "Set TEST_DATABASE_URL or DATABASE_URL before starting the integration server.",
  );
const databaseUrl = new URL(sourceUrl);
databaseUrl.pathname = `/${process.env.INTEGRATION_DATABASE_NAME ?? "k_coffee_test"}`;
if (!databaseUrl.pathname.includes("test"))
  throw new Error(
    "The integration server requires a database name containing 'test'.",
  );
